...
class JobQueue {
    ...
    defaultRetry;
    activeWorkers = 0;
    // fix: track pending (spawned-but-not-yet-active) workers to prevent over-spawning under burst
    pendingWorkers = 0;
    isClosed = false;
    isPaused = false;
    drainResolvers = [];
    ...
    constructor(config) {
        ...
        if (dlqPlugin) {
            ...
            // feat: wire WAL to DLQ so entries survive crashes
    ...
    spawnWorker() {
        // fix: increment pendingWorkers synchronously before the async loop starts
        this.pendingWorkers++;
    ...
    triggerWorker() {
        if (this.isClosed || this.isPaused)
            return;
        // fix: guard with pendingWorkers + activeWorkers to prevent over-spawning under burst
        if (this.activeWorkers + this.pendingWorkers < this.cfg.workers.max) {
            this.spawnWorker();
        }
    }
    async runWorkerLoop() {
        // fix: transition pending → active at the start of the loop
    ...
    checkDrain() {
        ...
        if (this.activeWorkers === 0 && this.pendingWorkers === 0 && this.scheduler.size === 0) {
