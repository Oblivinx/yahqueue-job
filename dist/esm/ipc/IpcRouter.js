...
export class IpcRouter {
    ...
    registerShard(shardKey, child) {
        ...
        child.on('message', (msg) => {
            if (msg.reqId && this.pendingRequests.has(msg.reqId)) {
                ...
                if (msg.error)
                    reject(new QueueError(`IPC Error: ${msg.error}`));
                else
                    resolve(msg.payload);
    ...
    async enqueue(options) {
        ...
        if (!shardKey)
            throw new QueueError('enqueue() via IpcRouter requires a shardKey');
        const child = this.shards.get(shardKey);
        if (!child)
            throw new QueueError(`No shard registered for key: ${shardKey}`);
    ...
    sendRequest(child, cmd, payload) {
        return new Promise((resolve, reject) => {
            if (!child.connected) {
                return reject(new QueueError(`IPC Channel closed for shard`));
            }
            ...
            setTimeout(() => {
                if (this.pendingRequests.has(reqId)) {
                    ...
                    reject(new QueueError(`IPC Request timeout for cmd: ${cmd}`));
                }
            }, 5000);
